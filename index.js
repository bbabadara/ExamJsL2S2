...
document.addEventListener("DOMContentLoaded", async (event) => {
  ...
  searchArticle.addEventListener("input", function () {
    if (searchArticle.value.trim() != "") {
      ...
      let resultat = articles.filter((a) =>
        a.libelle.toLowerCase().includes(recherche)
      );
...
function generateTr2(d) {
  return ` <tr data-id="${d.id}">
                            ...
                            <td><button class="text-red-500" onclick="removeArticle(${d.id})">
...
function selectArticle(element) {
  if (element.checked) {
    if (!tBody2.querySelector(`tr[data-id="${element.dataset.id}"]`)) {
      tBody2.innerHTML += generateTr2(findArticleById(element.dataset.id));
    }
  } else {
    // Retirer l'article si la case est décochée
    const rowToRemove = tBody2.querySelector(`tr[data-id="${element.dataset.id}"]`);
    if (rowToRemove) {
      rowToRemove.remove();
    }
  }
...
function removeArticle(id) {
    const rowToRemove = tBody2.querySelector(`tr[data-id="${id}"]`);
    if (rowToRemove) {
        rowToRemove.remove();
    }
}
...
function findArticlesByInputSearch(input) {
  return articles.filter((a) =>
    a.libelle.toLowerCase().includes(input.toLowerCase())
  );
}
...
function generateInfoClient(client) {
  const montantTotal = client.dette.reduce(
